...
# KFolded DataSplitter class
class DataSplitterWithIndex(pl.LightningDataModule):
    """
    A variant of DataSplitter that accepts pre-defined indices for train, validation, and test sets.
    
    Parameters
    ----------
    adata_manager : AnnDataManager
        The AnnDataManager object.
    train_idx : np.ndarray
        Array of indices to use for the training set.
    val_idx : np.ndarray
        Array of indices to use for the validation set.
    test_idx : np.ndarray
        Array of indices to use for the test set.
    load_sparse_tensor : bool, optional
        Whether to load sparse tensors. Default is False.
    pin_memory : bool, optional
        Whether to pin memory for data loaders. Default is False.
    **kwargs
        Additional keyword arguments for the data loaders.
    """
    ...
    def setup(self, stage: Optional[str] = None):
        """Sets the indices for train, validation, and test sets."""
        if self.shuffle_set_split:
        ...
        self.n_val = len(self.val_idx) if self.val_idx is not None else 0
        self.n_test = len(self.test_idx) if self.test_idx is not None else 0
