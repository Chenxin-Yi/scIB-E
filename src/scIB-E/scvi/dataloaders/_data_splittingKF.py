...
class KFoldDataSplitter(pl.LightningDataModule):
    """
    ...
    ----------
    adata_manager
        An instance of :class:`~scvi.data.AnnDataManager` created via `setup_anndata`.
    n_splits
    ...
    **kwargs
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.data_loader_kwargs = kwargs
        self.pin_memory = pin_memory
        self.kf = KFold(n_splits=self.n_splits, shuffle=self.shuffle, random_state=42)

    def setup(self, stage: Optional[str] = None):
        """Split indices in train/val sets."""
        self.indices = np.arange(self.adata_manager.adata.n_obs)
        self.splits = list(self.kf.split(self.indices))
        self.dataset = self.adata_manager.create_torch_dataset()  # 初始化数据集

    def set_current_fold(self, fold_idx: int):
        """Set the current fold index."""
        self.current_fold = fold_idx
        self.train_idx, self.val_idx = self.splits[self.current_fold]

    def train_dataloader(self):
        """Create train data loader."""
        # train_subset = Subset(self.dataset, self.train_idx)
        return self.data_loader_cls(
            self.adata_manager,
            indices=self.train_idx,
            shuffle=True,
    ...
    def val_dataloader(self):
        """Create validation data loader."""
        # val_subset = Subset(self.dataset, self.val_idx)
        return self.data_loader_cls(
            self.adata_manager,
            indices=self.val_idx,
            shuffle=False,
