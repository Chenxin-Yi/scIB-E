...
class VAE(EmbeddingModuleMixin, BaseMinifiedModeModuleClass):
    ...
    # Modifications
    batch_merge
        select the way to inject batch embeddings, only used in batch_representation == 'embedding'
    ...
    def __init__(
        ...
        encode_covariates: bool = False, # ljw: not inject to the inpute layer
        deeply_inject_covariates: bool = False, # ljw: not inject into encoder
        ...
        emb_merge: Literal["add", "cat"] = "cat", # embedding design
    ):
        ...
        self.emb_merge = emb_merge
        if self.batch_representation == "embedding":
            if self.emb_merge=="cat": 
                n_input_encoder += batch_dim * encode_covariates # default no encode in the input layer
            cat_list = list([] if n_cats_per_cov is None else n_cats_per_cov)
        else:
            cat_list = [n_batch] + list([] if n_cats_per_cov is None else n_cats_per_cov)

        encoder_cat_list = cat_list if encode_covariates else None # None
        _extra_encoder_kwargs = extra_encoder_kwargs or {} # None
        ...
        # library encoder goes from n_input-dimensional data to 1-d library size
        ...
        if self.batch_representation == "embedding" and self.emb_merge=="cat": 
        ...
        if self.batch_representation == "embedding":
            ...
            if self.emb_merge == "cat":
                if decoder_input.dim() == 3:
                    batch_rep = batch_rep.unsqueeze(0)  
                    batch_rep = batch_rep.expand(decoder_input.size(0), -1, -1)  # 变成 [25, 64, 30]
                decoder_input = torch.cat([decoder_input, batch_rep], dim=-1)
            else:
                decoder_input = torch.add(decoder_input, batch_rep)
