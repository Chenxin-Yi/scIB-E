...
# The HSIC component is adapted from
...
class HSIC_TrainingPlan(TrainingPlan):
    """Training plan for VAEs with Hilbert-Schmidt Independence Criterion (HSIC) loss to encourage latent space mixing.
    ...
    **loss_kwargs
    ...
    def kernel_matrix(self, x: torch.Tensor, sigma):
    ...
    def bandwidth(self,d):
        ...
        temp = torch.Tensor([loggamma(0.5 * (d + 1)) - loggamma(0.5 * d)]).cuda()
        gz = 2 * torch.exp(temp)
        return 1. / (2. * gz**2)
    ...
    def hsic(self,z, s):
        ...
        zz = self.kernel_matrix(z, self.bandwidth(d_z))
        ss = self.kernel_matrix(s, self.bandwidth(d_s))
    ...
    def training_step(self, batch, batch_idx):
        ...
        hsic_loss = self.batch_correction_weight * self.hsic(z, batch_embs)
        loss += hsic_loss
        
        self.log("HSIC_loss", hsic_loss, on_epoch=True, prog_bar=True)
