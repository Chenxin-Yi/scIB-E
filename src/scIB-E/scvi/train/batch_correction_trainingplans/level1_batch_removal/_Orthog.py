...
class Orthog_TrainingPlan(TrainingPlan):
    """Training plan for VAEs with Orthogonal Projection (Orthog) loss to encourage latent space mixing.
    ...
    **loss_kwargs
    ...
    def orthogonalization_loss(self, emb1, emb2):
        """Compute Supervised Contrastive Loss."""
        emb1_normalized = emb1 - emb1.mean(dim=0, keepdim=True)
        emb2_normalized = emb2 - emb2.mean(dim=0, keepdim=True)

        # Calculate the covariance matrix between all pairs of features from emb1 and emb2
        cov_matrix = torch.mm(emb1_normalized.T, emb2_normalized) / (emb1.size(0) - 1)

        # Compute the loss as the sum of the squared elements of the covariance matrix
        decorrelation_loss = (cov_matrix ** 2).sum()
    ...
    def training_step(self, batch, batch_idx):
        ...
        orthog_loss = self.batch_correction_weight * self.orthogonalization_loss(z, batch_embs)
        loss += orthog_loss
        ...
        for lbl in unique_batch_labels:
            ...
            # mask_matrix = self.create_combined_mask(batch_label[mask])
        ...
        self.log("Orthog_loss", orthog_loss, on_epoch=True, prog_bar=True)
